...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    const allowedOrigins = [
      ...
      "https://34.27.53.239",
      "https://35.184.62.157",
      "http://35.225.218.1", // Frontend LoadBalancer IP
      "https://35.225.218.1",
...
async function fetchWeather() {
  ...
  try {
    ...
    console.log(
      `[${timestamp}] Weather data saved: ${temp}°C, ${description}, Humidity: ${humidity}%, Pressure: ${pressure}hPa, Wind: ${windSpeed}m/s`
    );
  } catch (error) {
    const timestamp = new Date().toLocaleString("tr-TR");
...
app.listen(port, "0.0.0.0", () => {
  ...
  console.log(
    `Weather API Key: ${process.env.WEATHER_API_KEY ? "Set" : "Not Set"}`
  );
});
